...
export default function BeforeAfter({ title, caption, before, after, labels = {}, priority }: Props) {
  ...
  return (
    <figure className="work-card">
      ...
      >
        <Image src={urlFor(after).width(1100).url()} alt={`${title} — after`} fill draggable={false} priority={priority}
        ...
        <div className="ba-before" style={{ clipPath: `inset(0 ${100 - pct}% 0 0)` }}>
          <Image src={urlFor(before).width(1100).url()} alt={`${title} — before`} fill draggable={false} priority={priority}
            sizes="(max-width: 860px) 100vw, 50vw" />
        </div>
        <span className="pill-tag left">{labels.before ?? 'Before'}</span>
        <span className="pill-tag right">{labels.after ?? 'After'}</span>
        ...
        <span className="ba-hint">{labels.drag ?? 'Drag'}</span>
      </div>
      <figcaption>
        <h3>{title}</h3>
        {caption && <p>{caption}</p>}
